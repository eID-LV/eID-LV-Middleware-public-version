...
static int PKCS11_pkey_asn1_meths(ENGINE *e, EVP_PKEY_ASN1_METHOD **ameth,
	const int **nids, int nid)
{
	static int pkey_nids[] = {
		EVP_PKEY_RSA,
	...
	static EVP_PKEY_ASN1_METHOD *pkey_asn1_method_ec = NULL;

	(void)e; /* squash the unused parameter warning */
	/* all PKCS#11 engines currently share the same pkey_meths */

	if (!ameth) { /* get the list of supported nids */
		*nids = pkey_nids;
		return sizeof(pkey_nids) / sizeof(int) - 1;
	}

	/* get the EVP_PKEY_ASN1_METHOD */
	switch (nid) {
	case EVP_PKEY_RSA:
	/*	if (pkey_asn1_method_rsa == NULL)
			pkey_asn1_method_rsa = pkcs11_pkey_asn1_method_rsa();*/
		if (pkey_asn1_method_rsa == NULL)
			return 0;
...
#ifndef OPENSSL_NO_EC
	case EVP_PKEY_EC:
	/*	if (pkey_asn1_method_ec == NULL)
			pkey_asn1_method_ec = pkcs11_asn1_pkey_method_ec();*/
		if (pkey_asn1_method_ec == NULL)
			return 0;
		*ameth = pkey_asn1_method_ec;
		return 1; /* success */
#endif /* OPENSSL_NO_EC */
	}
	*ameth = NULL;
