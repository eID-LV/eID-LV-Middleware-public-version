...
int pkcs11_is_session_opened(PKCS11_SLOT *slot, int rw, int* res)
{
	PKCS11_SLOT_private *spriv = PRIVSLOT(slot);
	PKCS11_CTX *ctx = SLOT2CTX(slot);
	...
	rv = CRYPTOKI_call(ctx, C_GetSessionInfo(spriv->session, &info));
	...
	{
		if (((info.flags & CKF_RW_SESSION) && rw)
			|| (!(info.flags & CKF_RW_SESSION) && !rw)
			)
		{
			*res = 1;
		}
