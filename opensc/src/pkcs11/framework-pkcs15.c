...
#ifdef _WIN64
	unsigned int i = 0, retry_counter = 0;
...
#ifdef _WIN64
create_object_retry :
	/* ensure uniqueness of object handle */
	handle = ((CK_OBJECT_HANDLE)(uintptr_t)obj);
	for (i = 0; i < fw_data->num_objects; i++)
	{
		if (((CK_OBJECT_HANDLE)(uintptr_t)(fw_data->objects[i])) == handle)
		{
			break;
		}
	}

	if (i < fw_data->num_objects)
	{
		sc_log(context, "__pkcs15_create_object handle 0x%lx already exists. trying to create new one.", handle);

		if (retry_counter < 5)
		{
			/* backup pointer to free it after the new memory allocation
			 * this increases chances to have a pointer whose lowest 32-bit
			 * value doesn't exist in our list.
			 */
			void* ptr_backup = obj;
			obj = calloc(1, size);
			free(ptr_backup);
			if (!obj)
			{
				sc_log(context, "__pkcs15_create_object memory allocation failed. Quit!", handle);
				return SC_ERROR_OUT_OF_MEMORY;
			}
			retry_counter++;

			goto create_object_retry;
		}
		else
			free(obj);
			
		sc_log(context, "__pkcs15_create_object failed to find a new handle that doesn't already exist after %d attempts. Quit!", retry_counter);
		return SC_ERROR_OUT_OF_MEMORY;
	}
#endif
...
CK_RV C_GetTokenInfo(CK_SLOT_ID slotID, CK_TOKEN_INFO_PTR pInfo)
{
	...
	if (slot->p11card->token_not_recognized)
	{
		sc_log(context, "C_GetTokenInfo() token_not_recognized is set on p11card. Inserted card not recognized.");
	...
	if (!p15card)
	{
		sc_log(context, "C_GetTokenInfo() fw_data->p15_card is NULL on token. Inserted card not recognized.");
		rv = CKR_TOKEN_NOT_RECOGNIZED;
		goto out;
	}
	...
	if (auth) {
		...
		if (pin_info->logged_in != SC_PIN_STATE_NOT_INITIALIZED)
	...
	if ((r == SC_ERROR_OBJECT_NOT_FOUND) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SO_LOGGED_INITPIN))
		r = sc_pkcs15_find_unblocking_pin(p15card, &auth);
	if ((r == 0) && auth) 
	{
		pin_info = (struct sc_pkcs15_auth_info*) auth->data;
		...
		struct pkcs15_any_object **cert_object)
{
	...
	if (cert->flags & SC_PKCS15_CO_FLAG_PRIVATE)  {	/* is the cert private? */
		...
		p15_info->value.value = malloc(p15_cert->data.len);
		if (p15_info->value.value)
		{
			memcpy(p15_info->value.value, p15_cert->data.value, p15_cert->data.len);
			p15_info->value.len = p15_cert->data.len;
		}
	}
...
int slot_get_logged_in_state(struct sc_pkcs11_slot *slot)
{
	...
	if (slot->login_user == CKU_SO) {
		int r = sc_pkcs15_find_so_pin(p15card, &pin_obj);
		if ((r == SC_ERROR_OBJECT_NOT_FOUND) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SO_LOGGED_INITPIN))
			r = sc_pkcs15_find_unblocking_pin(p15card, &pin_obj);
	} else {
		...
		CK_CHAR_PTR pPin, CK_ULONG ulPinLen)
{
	...
	case CKU_SO:
		...
		if ((rc == SC_ERROR_OBJECT_NOT_FOUND) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SO_LOGGED_INITPIN))
			rc = sc_pkcs15_find_unblocking_pin(p15card, &auth_object);
...
pkcs15_logout(struct sc_pkcs11_slot *slot)
{
	...
	case CKU_SO:
		...
		if ((rc == SC_ERROR_OBJECT_NOT_FOUND) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SO_LOGGED_INITPIN))
			rc = sc_pkcs15_find_unblocking_pin(p15card, &auth_object);
		sc_log(context, "pkcs15-logout: find SO PIN: rc %i", rc);
	...
	rc = sc_pkcs15_logout_pin(p15card, auth_object);
	sc_log(context, "PKCS15 logout PIN returned %d", rc);
		...
		CK_CHAR_PTR pNewPin, CK_ULONG ulNewLen)
{
	...
	if (login_user == CKU_SO) {
		...
		if ((rc == SC_ERROR_OBJECT_NOT_FOUND) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SO_LOGGED_INITPIN))
			rc = sc_pkcs15_find_unblocking_pin(p15card, &pin_obj);
		sc_log(context, "pkcs15-login: find SO PIN: rc %i", rc);
	} else {
		pin_obj = slot_data_auth(slot->fw_data);
	}
	...
	if (	((login_user < 0) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_UNLOGGED_SETPIN))
		||	((login_user == CKU_CONTEXT_SPECIFIC) && (sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SCONTEXT_SETPIN))
		)
	{
		rc = sc_pkcs15_find_so_pin(p15card, &puk_obj);
		if (rc == SC_ERROR_OBJECT_NOT_FOUND)
			rc = sc_pkcs15_find_unblocking_pin(p15card, &puk_obj);
		if (rc != 0)
		{
			puk_obj = NULL;
			sc_debug(context, SC_LOG_DEBUG_NORMAL, "No SOPIN found");
		}
		rc = sc_pkcs15_unblock_pin(fw_data->p15_card, puk_obj, pin_obj, pOldPin, ulOldLen, pNewPin, ulNewLen);
	}
...
pkcs15_init_pin(struct sc_pkcs11_slot *slot, CK_CHAR_PTR pPin, CK_ULONG ulPinLen)
{
	...
	struct sc_pkcs15_object	*so_auth_obj = NULL;
	struct sc_pkcs15_auth_info *auth_info = NULL;
	struct sc_pkcs15_auth_info *so_auth_info = NULL;
	...
	if (auth_info && sc_pkcs11_conf.pin_unblock_style == SC_PKCS11_PIN_UNBLOCK_SO_LOGGED_INITPIN)   {
		...
		if (rc == SC_ERROR_OBJECT_NOT_FOUND)
			rc = sc_pkcs15_find_unblocking_pin(p15card, &so_auth_obj);
		if (rc != 0)
		{
		...
		/* check that the new PIN value respects PIN lengths constraints */
		if (!(p11card->card->reader->capabilities & SC_READER_CAP_PIN_PAD)
			&& (ulPinLen < auth_info->attrs.pin.min_length || ulPinLen > auth_info->attrs.pin.max_length)
			)
		{
			return CKR_PIN_LEN_RANGE;
		}
		...
		rc = sc_pkcs15_unblock_pin(fw_data->p15_card, so_auth_obj, auth_obj,
			(fw_data->user_puk_len)? fw_data->user_puk : NULL, fw_data->user_puk_len, pPin, ulPinLen);
		...
		CK_BYTE_PTR pData, CK_ULONG_PTR pulDataLen)
{
	...
	modulus_bytes_length = (prkey->prv_info->modulus_length + 7) / 8;
	...
	case CKM_RSA_PKCS:
		...
		if (!pData)
		{
			*pulDataLen = (CK_LONG)modulus_bytes_length - 11;
			sc_debug(context, SC_LOG_DEBUG_NORMAL, "returning maximum possible length of output (%d bytes).\n", *pulDataLen);
			return CKR_OK;
		}
		break;
	case CKM_RSA_X_509:
		...
		if (!pData)
		{
			*pulDataLen = (CK_LONG)modulus_bytes_length;
			sc_debug(context, SC_LOG_DEBUG_NORMAL, "returning maximum possible length of output (%d bytes).\n", *pulDataLen);
			return CKR_OK;
		}
		else if (*pulDataLen < (CK_ULONG)modulus_bytes_length)
		{
			*pulDataLen = (CK_LONG)modulus_bytes_length;
			sc_debug(context, SC_LOG_DEBUG_NORMAL, "output buffer is too small (less than %d bytes).\n", *pulDataLen);
			return CKR_BUFFER_TOO_SMALL;
		}
		break;
	default:
	...
	/* encrypted data must have the same length as modulus */
	if ((size_t)ulEncryptedDataLen != modulus_bytes_length)
	{
	...
	if (prkey->pub_data)
	{
		/* encrypted data representation as an integer must be smaller than modulus */
		CK_BYTE_PTR pModulus = (CK_BYTE_PTR)prkey->pub_data->u.rsa.modulus.data;
		for (i = 0; i < modulus_bytes_length; i++)
		{
			if (pEncryptedData[i] > pModulus[i])
			{
		...
		if (i == modulus_bytes_length)
		{
			/* encrypted data equal to the modulus */
			return CKR_ENCRYPTED_DATA_INVALID;
		}
	}
		...
		CK_MECHANISM_TYPE mech_type, unsigned int flags)
{
	...
	for (ii=0;ii<SC_MAX_SUPPORTED_ALGORITHMS && pkinfo->algo_refs[ii];ii++)   {
		...
		{
			/* in case of ECDSA mechanism, we allow operation to proceed if
			 * key can sign bigger hash than the one specified and also if 
			 * the EC key size is bigger or equal to the mechanism hash
			 */
			if ((mech_type >= CKM_ECDSA) && (mech_type <= CKM_ECDSA_SHA512)
				&& ((token_algos + jj)->mechanism >= CKM_ECDSA) && (((token_algos + jj)->mechanism <= CKM_ECDSA_SHA512))
				&& (	((token_algos + jj)->mechanism == CKM_ECDSA) 
					||	((token_algos + jj)->mechanism > mech_type)
					||	(mech_type == CKM_ECDSA_SHA1 && pkinfo->field_length >= 160)
					||	(mech_type == CKM_ECDSA_SHA224 && pkinfo->field_length >= 224)
					||	(mech_type == CKM_ECDSA_SHA256 && pkinfo->field_length >= 256)
					||	(mech_type == CKM_ECDSA_SHA384 && pkinfo->field_length >= 384)
					||	(mech_type == CKM_ECDSA_SHA512 && pkinfo->field_length >= 512)
					)
				)
			{
				/* we allow this mechanism*/
			}
			/* in case of CKM_RSA_PKCS mechanism, we allow operation to proceed if
			 * key can sign using CKM_RSA_SHAXXX_PKCS */
			else if ((flags == CKF_SIGN) && (mech_type == CKM_RSA_PKCS)
				&& (((token_algos + jj)->mechanism == CKM_SHA1_RSA_PKCS)
					|| ((token_algos + jj)->mechanism == CKM_SHA224_RSA_PKCS)
					|| ((token_algos + jj)->mechanism == CKM_SHA256_RSA_PKCS)
					|| ((token_algos + jj)->mechanism == CKM_SHA384_RSA_PKCS)
					|| ((token_algos + jj)->mechanism == CKM_SHA512_RSA_PKCS)
					)
				)
			{
				/* we allow this mechanism*/
			}
			/* in case of CKM_RSA_SHAXXX_PKCS mechanism, we allow operation to proceed if
			 * key can sign using CKM_RSA_PKCS */
			else if ((flags == CKF_SIGN) && ((token_algos + jj)->mechanism == CKM_RSA_PKCS)
				&& (mech_type == CKM_SHA1_RSA_PKCS || mech_type == CKM_SHA224_RSA_PKCS || mech_type == CKM_SHA256_RSA_PKCS
					|| mech_type == CKM_SHA384_RSA_PKCS || mech_type == CKM_SHA512_RSA_PKCS)
				)
			{
				/* we allow this mechanism*/
			}
...
pkcs15_pubkey_get_attribute(struct sc_pkcs11_session *session, void *object, CK_ATTRIBUTE_PTR attr)
{
	...
	case CKA_DERIVE:
		if (pubkey->pub_info)
			...
			return get_usage_bit(SC_PKCS15_PRKEY_USAGE_VERIFY, attr);
		else
...
register_mechanisms(struct sc_pkcs11_card *p11card)
{
	...
	if (rsa_flags & SC_ALGORITHM_RSA_PAD_PKCS1) {
		...
		if (rsa_flags & SC_ALGORITHM_RSA_HASH_SHA224) {
			rc = sc_pkcs11_register_sign_and_hash_mechanism(p11card, CKM_SHA224_RSA_PKCS, CKM_SHA224, mt);
			if (rc != CKR_OK)
				return rc;
		}
