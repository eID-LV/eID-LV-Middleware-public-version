				...
				CK_ULONG_PTR pulCount)
{
	...
	if (p11card->token_not_recognized)
		...
		CK_BYTE_PTR pSignature, CK_ULONG_PTR pulSignatureLen)
{
	struct signature_data *data;
	CK_RV rv;
	CK_MECHANISM op_mech = operation->mechanism;
	unsigned int card_mech;
	...
	if (data->md) {
		...
		rv = md->type->md_final(md, digest, &len);
		if (rv == CKR_BUFFER_TOO_SMALL)
		...
		case CKM_SHA512_RSA_PKCS:
			if (op_mech.mechanism == CKM_SHA1_RSA_PKCS)
				card_mech = SC_ALGORITHM_RSA_HASH_SHA1;
			else if (op_mech.mechanism == CKM_SHA224_RSA_PKCS)
				card_mech = SC_ALGORITHM_RSA_HASH_SHA224;
			else if (op_mech.mechanism == CKM_SHA256_RSA_PKCS)
				card_mech = SC_ALGORITHM_RSA_HASH_SHA256;
			else if (op_mech.mechanism == CKM_SHA384_RSA_PKCS)
				card_mech = SC_ALGORITHM_RSA_HASH_SHA384;
			else if (op_mech.mechanism == CKM_SHA512_RSA_PKCS)
				card_mech = SC_ALGORITHM_RSA_HASH_SHA512;
			/* add digest info*/
			data->buffer_len = sizeof(data->buffer);
			if (sc_pkcs1_add_digest_info_prefix(card_mech, digest, len, data->buffer, &data->buffer_len) < 0)
			{
				LOG_FUNC_RETURN(context, (int) SC_ERROR_INTERNAL);
			}
			op_mech.mechanism = CKM_RSA_PKCS;
			break;
		case CKM_ECDSA_SHA1:
		...
		case CKM_ECDSA_SHA512:
			op_mech.mechanism = CKM_ECDSA;
			/* on purpose fall in*/
		default:
			memcpy(data->buffer, digest, len);
			data->buffer_len = (unsigned int)len;
			break;
		}
	}

	rv = data->key->ops->sign(operation->session, data->key, &op_mech,
