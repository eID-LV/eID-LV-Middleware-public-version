...
{				/* receives new session handle */
	...
	if (flags & ~(CKF_SERIAL_SESSION | CKF_RW_SESSION | CKF_IMPLICIT_SO_SESSION))
...
session_retry:
	...
	if (list_seek(&sessions, &session->handle) != NULL) {
		...
		if (retry_counter < 5)
		{
			 ...
			 */
			void* ptr_backup = session;
			sc_log(context, "C_OpenSession: trying to create new one.");
			session = (struct sc_pkcs11_session *)calloc(1, sizeof(struct sc_pkcs11_session));
			free(ptr_backup);
			retry_counter++;
			goto session_retry;
		...
		sc_log(context, "C_OpenSession failed to find a new handle that doesn't already exist after %d attempts. Quit!", retry_counter);
#else
		free(session);
#endif
...
{				/* receives session information */
	...
	slot = session->slot;
	state = slot_get_logged_in_state(slot);
	logged_out = (state == SC_PIN_STATE_LOGGED_OUT || state == SC_PIN_STATE_NOT_INITIALIZED);
	       ...
	       CK_CHAR_PTR pOldPin, CK_ULONG ulOldLen, CK_CHAR_PTR pNewPin, CK_ULONG ulNewLen)
{
	...
	if (rv == CKR_OK)
	{
		...
		/* set logged in user to SO temporarly if CKF_IMPLICIT_SO_SESSION set and no one is logged */
		if ((login_user == -1) && (session->flags & CKF_IMPLICIT_SO_SESSION))
			slot->login_user = CKU_SO;
		rv = slot->p11card->framework->change_pin(slot, pOldPin, ulOldLen, pNewPin, ulNewLen);
		/* restore login_user*/
		slot->login_user = login_user;
	}
	rv = reset_login_state(slot, rv);
