...
static void print_prkey_info(const struct sc_pkcs15_object *obj)
{
	...
	printf("\tNative         : %s\n", prkey->native ? "yes" : "no");
	if (prkey->algo_refs[0])
	{
		int index;
		printf("\tAlgo refs      : 0x%.8X", prkey->algo_refs[0]);
		for (index = 1; index < SC_MAX_SUPPORTED_ALGORITHMS && prkey->algo_refs[index]; index++)
			printf(", 0x%.8X", prkey->algo_refs[index]);
		printf("\n");
	}
...
static int unblock_pin(void)
{
	...
	sc_pkcs15_object_t *pin_obj, *puk_obj = NULL;
	u8 *pin, *puk;
	...
	if (!puk_obj)   {
		...
		if (r == SC_ERROR_OBJECT_NOT_FOUND)
			r = sc_pkcs15_find_unblocking_pin(p15card, &puk_obj);
		if (r != 0)
		{
			return 2;
		}
