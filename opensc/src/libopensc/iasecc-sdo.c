...
iasecc_sdo_free_fields(struct sc_card *card, struct iasecc_sdo *sdo)
{
	...
	if (sdo->sdo_class == IASECC_SDO_CLASS_RSA_PUBLIC)   {
		if (sdo->data.pub_key.rsa.n.value)
			free(sdo->data.pub_key.rsa.n.value);
		if (sdo->data.pub_key.rsa.e.value)
			free(sdo->data.pub_key.rsa.e.value);
		if (sdo->data.pub_key.compulsory.value)
			free(sdo->data.pub_key.compulsory.value);
		if (sdo->data.pub_key.rsa.chr.value)
			free(sdo->data.pub_key.rsa.chr.value);
		if (sdo->data.pub_key.rsa.cha.value)
			free(sdo->data.pub_key.rsa.cha.value);
	}
	else if (sdo->sdo_class == IASECC_SDO_CLASS_RSA_PRIVATE)   {
		if (sdo->data.prv_key.rsa.p.value)
			free(sdo->data.prv_key.rsa.p.value);
		if (sdo->data.prv_key.rsa.q.value)
			free(sdo->data.prv_key.rsa.q.value);
		if (sdo->data.prv_key.rsa.iqmp.value)
			free(sdo->data.prv_key.rsa.iqmp.value);
		if (sdo->data.prv_key.rsa.dmp1.value)
			free(sdo->data.prv_key.rsa.dmp1.value);
		if (sdo->data.prv_key.rsa.dmq1.value)
			free(sdo->data.prv_key.rsa.dmq1.value);
		if (sdo->data.prv_key.compulsory.value)
			free(sdo->data.prv_key.compulsory.value);
	}
	else if (sdo->sdo_class == IASECC_SDO_CLASS_EC_PUBLIC) {
		if (sdo->data.pub_key.ec.oid.value)
			free(sdo->data.pub_key.ec.oid.value);
		if (sdo->data.pub_key.ec.p.value)
			free(sdo->data.pub_key.ec.p.value);
		if (sdo->data.pub_key.ec.a.value)
			free(sdo->data.pub_key.ec.a.value);
		if (sdo->data.pub_key.ec.b.value)
			free(sdo->data.pub_key.ec.b.value);
		if (sdo->data.pub_key.ec.g.value)
			free(sdo->data.pub_key.ec.g.value);
		if (sdo->data.pub_key.ec.n.value)
			free(sdo->data.pub_key.ec.n.value);
		if (sdo->data.pub_key.ec.q.value)
			free(sdo->data.pub_key.ec.q.value);
		if (sdo->data.pub_key.ec.h.value)
			free(sdo->data.pub_key.ec.h.value);
		if (sdo->data.prv_key.compulsory.value)
			free(sdo->data.prv_key.compulsory.value);
	}
	else if (sdo->sdo_class == IASECC_SDO_CLASS_EC_PRIVATE) {
		if (sdo->data.prv_key.ec.oid.value)
			free(sdo->data.prv_key.ec.oid.value);
		if (sdo->data.prv_key.ec.d.value)
			free(sdo->data.prv_key.ec.d.value);
		if (sdo->data.prv_key.ec.p.value)
			free(sdo->data.prv_key.ec.p.value);
		if (sdo->data.prv_key.ec.a.value)
			free(sdo->data.prv_key.ec.a.value);
		if (sdo->data.prv_key.ec.b.value)
			free(sdo->data.prv_key.ec.b.value);
		if (sdo->data.prv_key.ec.g.value)
			free(sdo->data.prv_key.ec.g.value);
		if (sdo->data.prv_key.ec.n.value)
			free(sdo->data.prv_key.ec.n.value);
		if (sdo->data.prv_key.ec.h.value)
			free(sdo->data.prv_key.ec.h.value);
		if (sdo->data.prv_key.compulsory.value)
			free(sdo->data.prv_key.compulsory.value);
	}
...
iasecc_crt_parse(struct sc_card *card, unsigned char *data, struct iasecc_se_info *se)
{
	struct sc_context *ctx = card->ctx;
	struct sc_crt crt;
	int ii, offs, len, id_len, parsed_len = -1;
	...
	for(offs = 2; offs < len;)   {
		...
		if (*(data + offs) == IASECC_CRT_TAG_USAGE)   {
			...
			id_len = (int) *(data + offs + 1);
			if ((offs + 1 + id_len) < (len + 2) && ((id_len == 1) || (id_len == 4)))
			{
				memcpy(crt.algo.algo_id, data + offs + 2, id_len);
				crt.algo.algo_id_len = id_len;
				offs += 2 + id_len;				
			}
			else
				LOG_FUNC_RETURN(ctx, SC_ERROR_INVALID_DATA);
...
iasecc_se_get_crt(struct sc_card *card, struct iasecc_se_info *se, struct sc_crt *crt)
{
	...
	for (ii=0; ii<SC_MAX_CRTS_IN_SE && se->crts[ii].tag; ii++)   {
		if (crt->tag != se->crts[ii].tag)
			continue;
		if (crt->algo.algo_id_len && ((crt->algo.algo_id_len != se->crts[ii].algo.algo_id_len) || (memcmp (crt->algo.algo_id, se->crts[ii].algo.algo_id, crt->algo.algo_id_len))))
			continue;
...
iasecc_parse_prvkey(struct sc_card *card, unsigned char *data, size_t data_len, struct iasecc_sdo_prvkey *prvkey)
{
	...
	while(offs < data_len)   {
		...
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_OID)
			prvkey->ec.oid = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_P)
			prvkey->ec.p = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_A)
			prvkey->ec.a = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_B)
			prvkey->ec.b = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_G)
			prvkey->ec.g = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_N)
			prvkey->ec.n = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_H)
			prvkey->ec.h = tlv;
		else
...
iasecc_parse_pubkey(struct sc_card *card, unsigned char *data, size_t data_len, struct iasecc_sdo_pubkey *pubkey)
{
	...
	while(offs < data_len)   {
		...
		if (tlv.tag == IASECC_SDO_PUBKEY_TAG_N)
			...
			pubkey->ec.n = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_Q)
			pubkey->ec.q = tlv;
		else if (tlv.tag == IASECC_SDO_ECKEY_TAG_H)
			pubkey->ec.h = tlv;
...
iasecc_sdo_parse_data(struct sc_card *card, unsigned char *data, struct iasecc_sdo *sdo)
{
	...
	if (tlv.tag == IASECC_DOCP_TAG)   {
		...
		if (sdo->sdo_class != IASECC_SDO_CLASS_RSA_PRIVATE && sdo->sdo_class != IASECC_SDO_CLASS_EC_PRIVATE)
			LOG_TEST_RET(ctx, SC_ERROR_INVALID_DATA, "parse error: SDO_PRIVATE_KEY tag in non PRIVATE_KEY SDO");

		sdo->data.prv_key.is_ecc = (sdo->sdo_class == IASECC_SDO_CLASS_EC_PRIVATE) ? 1 : 0;
		...
		struct iasecc_sdo_update *sdo_update)
{
	...
	if (sdo->sdo_class == IASECC_SDO_CLASS_RSA_PRIVATE)   {
		...
		if (sdo->data.pub_key.rsa.cha.value)   {
			...
			sdo_update->fields[indx].value = sdo->data.pub_key.rsa.cha.value;
			sdo_update->fields[indx].size = sdo->data.pub_key.rsa.cha.size;
			indx++;
		}
		...
		if (sdo->data.pub_key.rsa.chr.value)   {
			...
			sdo_update->fields[indx].value = sdo->data.pub_key.rsa.chr.value;
			sdo_update->fields[indx].size = sdo->data.pub_key.rsa.chr.size;
			indx++;
		}
