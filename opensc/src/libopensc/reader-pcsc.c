...
static int refresh_attributes(sc_reader_t *reader)
{
...
#ifdef _WIN32
	if (priv->gpriv->SCardBeginTransaction == NULL)
	{
		/* minidriver case */
		priv->reader_state.dwCurrentState = SCARD_STATE_PRESENT;
		priv->reader_state.dwEventState = SCARD_STATE_PRESENT;
		reader->flags = SC_READER_CARD_INUSE | SC_READER_CARD_PRESENT;
		priv->reader_state.cbAtr = sizeof(priv->reader_state.rgbAtr);
		rv = priv->gpriv->SCardGetAttrib(priv->pcsc_card, SCARD_ATTR_ATR_STRING, (LPBYTE)priv->reader_state.rgbAtr, &priv->reader_state.cbAtr);
		if (rv != SCARD_S_SUCCESS)
		{
			PCSC_TRACE(reader, "SCardGetAttrib failed", rv);
	...
	if (SCARD_S_SUCCESS != priv->gpriv->SCardIsValidContext(priv->gpriv->pcsc_ctx))
	...
	if (rv != SCARD_S_SUCCESS) {
		if (rv == (LONG)SCARD_E_TIMEOUT) {
			...
			if ((priv->reader_state.dwEventState & SCARD_STATE_PRESENT) && !(reader->flags & SC_READER_CARD_PRESENT)
				&& !(priv->reader_state.dwEventState & SCARD_STATE_MUTE))
			{
				priv->reader_state.dwCurrentState &= ~SCARD_STATE_PRESENT;
...
#ifndef _WIN32
			else if ((priv->reader_state.dwEventState & SCARD_STATE_PRESENT) && (reader->flags & SC_READER_CARD_PRESENT)
				&& ((priv->reader_state.dwEventState & 0xFFFF0000) == 0) && (priv->pcsc_card))
			{
				/*
				 * If a card is present and high word of event state is 0, then we can't be sure that card has not changed
				 * In this case, we let the code continues its execution so that it reaches the part where we check
				 * if the card handle is still valid
				 */
			}
#endif
			else
	...
	if (	!(reader->flags & SC_READER_CARD_PRESENT)
		&&	(	(((priv->reader_state.dwEventState & (SCARD_STATE_PRESENT | SCARD_STATE_MUTE)) == (SCARD_STATE_PRESENT | SCARD_STATE_MUTE)) &&	(priv->reader_state.dwCurrentState & (SCARD_STATE_EMPTY)))
			||	(((priv->reader_state.dwCurrentState & (SCARD_STATE_PRESENT | SCARD_STATE_MUTE)) == (SCARD_STATE_PRESENT | SCARD_STATE_MUTE)) &&	(priv->reader_state.dwEventState & (SCARD_STATE_EMPTY)))
			)
		)
	{
		/* mute card: no event */
...
#else
				if ((rv != (LONG)SCARD_S_SUCCESS) && (rv != (LONG)SCARD_W_RESET_CARD))
#endif
				{
					/* get correct ATR since on OSX 10.14 SCardGetStatusChange return old ATR */
					SCARD_READERSTATE newState = {0};
					newState.szReader = reader->name;
					newState.dwCurrentState = SCARD_STATE_UNAWARE;
					newState.dwEventState = SCARD_STATE_UNAWARE;
					if (SCARD_S_SUCCESS == priv->gpriv->SCardGetStatusChange(priv->gpriv->pcsc_ctx, 0, &newState, 1))
					{
						if (newState.dwEventState & SCARD_STATE_PRESENT)
						{
							priv->reader_state.cbAtr = newState.cbAtr;
							reader->atr.len = newState.cbAtr;
							memcpy (priv->reader_state.rgbAtr, newState.rgbAtr, newState.cbAtr);
							memcpy(reader->atr.value, newState.rgbAtr, newState.cbAtr);
						}
					}
...
static int pcsc_lock(sc_reader_t *reader)
{
	...
	/* Don't use PCSC transaction on debug mode under Windows
	 * since taking a transaction for more than 5 seconds without
	 * sending APDUs cause the card to be reset
	 */
	rv = SCARD_S_SUCCESS;
#else
...
#ifdef _WIN32
		gpriv->SCardIsValidContext = (SCardIsValidContext_t)sc_dlsym(gpriv->dlhandle, "SCardIsValidContext");
#endif
...
static void detect_reader_features(sc_reader_t *reader, SCARDHANDLE card_handle) {
	...
	if (priv->get_tlv_properties) {
		 ...
		 * detected max_data */
		max_data = part10_detect_max_data(reader, card_handle);
	...
	if(gpriv->SCardGetAttrib != NULL) {
		...
		if (gpriv->SCardGetAttrib(card_handle, SCARD_ATTR_VENDOR_NAME,
			...
			if (rcount > ((sizeof rbuf)-1))
				rcount = (sizeof rbuf)-1;
			rbuf[rcount] = '\0';
...
static int pcsc_detect_readers(sc_context_t *ctx)
{
...
#ifdef _WIN32
		else if (SCARD_S_SUCCESS != gpriv->SCardIsValidContext(gpriv->pcsc_ctx))
		{
			/* detect case of loosing PC/SC context
			 * for example in case of RDP disconnect
			 */
			rv = SCARD_E_INVALID_HANDLE;
		}
#endif
	...
	for (reader_name = reader_buf; *reader_name != '\x0';
		...
		ret = pcsc_add_reader(ctx, reader_name, strlen(reader_name), (SCARDHANDLE) 0, &reader);
		if (ret != SC_SUCCESS) {
...
static int
part10_check_pin_min_max(sc_reader_t *reader, int pin_maxLength, struct sc_pin_cmd_data *data)
{
	struct pcsc_private_data *priv = reader->drv_data;
    int pin_minLength = reader->pin_minLength;

	if (priv->gpriv->fixed_pinlength != 0) {
		data->pin1.min_length = priv->gpriv->fixed_pinlength;
		data->pin1.max_length = priv->gpriv->fixed_pinlength;
		data->pin2.min_length = priv->gpriv->fixed_pinlength;
		data->pin2.max_length = priv->gpriv->fixed_pinlength;
		return 0;
	}
    ...
    if (pin_minLength > 0)
	{
		...
		if (data->pin2.min_length < value)
			data->pin2.min_length = value;
	}
	...
	if (pin_maxLength > 0)
	{
		...
		if (data->pin2.max_length > value)
			data->pin2.max_length = value;
	}
...
pcsc_pin_cmd(sc_reader_t *reader, struct sc_pin_cmd_data *data)
{
	...
	DWORD ioctl = 0;
	sc_apdu_t *apdu;
    int readerHasMaxPinLength = (reader->pin_maxLength <= 0)? 0 : 1;
...
retry_pin_cmd:
	...
	case SC_PIN_CMD_VERIFY:
		...
		part10_check_pin_min_max(reader, pin_maxLength, data);
	...
	case 0x6401: /* Input cancelled */
		if (((c2 - c1) < (CLOCKS_PER_SEC / 4)) && !readerHasMaxPinLength && (data->pin1.max_length > 8))
		{
			/* cancel response received in less than 250ms. This indicates firmware 
			 * rejected command and not real cancel by user.
			 */
			/* we retry with smaller max length values till we find the rigth one */
			int limit;
			if (data->pin1.max_length > 64)
			...
			pin_maxLength = limit;
			sc_debug(reader->ctx, SC_LOG_DEBUG_NORMAL, "PC/SC v2 pinpad: block transmit failed (status 0x6401). retrying with a new max length of %d\n", reader->pin_maxLength);
			goto retry_pin_cmd;
		}
...
int pcsc_use_reader(sc_context_t *ctx, void * pcsc_context_handle, void * pcsc_card_handle)
{
	...
	if (sc_ctx_get_reader_count(ctx) > 0) {
		oldrdr = list_get_at(&ctx->readers, 0);
	}
	...
	if(SCARD_S_SUCCESS == gpriv->SCardGetAttrib(card_handle,
		...
		/* if we already had a reader with a different name, delete it */
		if (oldrdr && strcmp(oldrdr->name, reader_name))
		{
			list_extract_at(&ctx->readers, 0);
		...
		{
			sc_reader_t *reader = NULL;
			ret = pcsc_add_reader(ctx, reader_name, reader_name_size, card_handle, &reader);
	...
	{
		/* something wrong happened. Just update the card handle in the existing reader instance */
		if(oldrdr)
		{
			struct pcsc_private_data *priv = oldrdr->drv_data;
			priv->pcsc_card = card_handle;
		}
	}
