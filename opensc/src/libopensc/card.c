...
static sc_card_t * sc_card_new(sc_context_t *ctx)
{
	...
	if (card == NULL)
		return NULL;
	card->ops = calloc(1, sizeof(struct sc_card_operations));
	if (card->ops == NULL) {
		free(card);
		return NULL;
	}
...
int sc_connect_card(sc_reader_t *reader, sc_card_t **card_out)
{
	...
	if (r && r != SC_ERROR_READER_LOCKED)
		goto err;

	if (r != SC_ERROR_READER_LOCKED)
		connected = 1;
	card->reader = reader;
	card->ctx = ctx;
				...
				*card->ops = *ops;
				...
				if (r) {
					...
					if (r == SC_ERROR_INVALID_CARD) {
						card->driver = NULL;
						card->name = NULL;
						memset(card->ops, 0, sizeof(struct sc_card_operations));
						continue;
					}
					goto err;
				}
	...
	*card_out = card;
    ...
    // We do not call sc_unlock here. It should be done in superior layers (P11 / TokenD / CTK), after card has been read.
...
int sc_disconnect_card(sc_card_t *card)
{
	...
	if (card->lock_count > 0)
	{
		...
		if (card->reader->ops->unlock != NULL)
			card->reader->ops->unlock(card->reader);
		card->lock_count = 0;
	}
...
int sc_lock(sc_card_t *card)
{
	...
	if (r == 0 && (was_reset > 0 || was_reattached > 0)) {
#ifdef ENABLE_SM
...
sc_card_sm_load(struct sc_card *card, const char *module_path, const char *in_module)
{
	...
	if (!module_path || strlen(module_path) == 0)   {
		temp_len = PATH_MAX-1;
		rv = sc_ctx_win32_get_config_value(NULL, "SmDir", "Software\\Latvia eID\\OpenSC",
				temp_path, &temp_len);
		if (rv == SC_SUCCESS) {
