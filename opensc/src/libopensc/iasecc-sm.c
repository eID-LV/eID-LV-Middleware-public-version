...
iasecc_sm_external_authentication(struct sc_card *card, unsigned skey_ref, int *tries_left)
{
	...
	cwa_session->params.crt_at.algo.algo_id_len = 1;
	cwa_session->params.crt_at.algo.algo_id[0] = IASECC_ALGORITHM_ROLE_AUTH;
...
iasecc_sm_se_mutual_authentication(struct sc_card *card, unsigned se_num)
{
	...
	offs = 0;
	sbuf[offs++] = IASECC_CRT_TAG_ALGO;
	sbuf[offs++] = crt->algo.algo_id_len;
	memcpy(&sbuf[offs], crt->algo.algo_id, crt->algo.algo_id_len);
	offs += crt->algo.algo_id_len;
	sbuf[offs++] = IASECC_CRT_TAG_REFERENCE;
	sbuf[offs++] = 0x01;
