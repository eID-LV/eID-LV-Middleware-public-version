...
sc_pkcs15_free_app(struct sc_pkcs15_card *p15card)
{
	if (p15card && p15card->app) {
		if (p15card->app->label) free(p15card->app->label);
		if (p15card->app->ddo.value) free(p15card->app->ddo.value);
		free(p15card->app);
		p15card->app = NULL;
	}
}
...
int
sc_pkcs15_find_unblocking_pin(struct sc_pkcs15_card *p15card, struct sc_pkcs15_object **out)
{
	struct sc_pkcs15_search_key sk;
