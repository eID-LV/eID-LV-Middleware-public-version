		...
		unsigned char **buf, size_t *buflen)
{
	...
	for (i=0; i<SC_MAX_SUPPORTED_ALGORITHMS && pubkey->algo_refs[i]; i++)   {
		sc_log(ctx, "Encode algorithm(%i) %i", i, pubkey->algo_refs[i]);
		sc_format_asn1_entry(asn1_supported_algorithms + i, &pubkey->algo_refs[i], NULL, 1);
	}
	sc_format_asn1_entry(asn1_com_key_attr + 5, asn1_supported_algorithms, NULL, pubkey->algo_refs[0] != 0);
