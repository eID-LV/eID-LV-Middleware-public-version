		...
		void *out, size_t *out_len)
{
	...
	if (!name_key)
		name_key = "Software\\Latvia eID\\OpenSC";
...
#ifdef _WIN32
	temp_len = PATH_MAX-1;
	r = sc_ctx_win32_get_config_value("LATVIAEID_CONF", "ConfigFile", "Software\\Latvia eID\\OpenSC",
		temp_path, &temp_len);
	if (r)   {
...
int sc_ctx_delete_all_readers(sc_context_t *ctx)
{
	assert(ctx != NULL);
...
int sc_context_create(sc_context_t **ctx_out, const sc_context_param_t *parm)
{
	...
	sc_log(ctx, "eID-LV Middleware: %s", sc_get_version());
...
int sc_get_cache_dir(sc_context_t *ctx, char *buf, size_t bufsize)
{
...
#ifndef _WIN32
	cache_dir = ".latviaeid/cache";
	homedir = getenv("HOME");
#else
	cache_dir = "Latvia-eID/cache";
	homedir = NULL;
	hr = SHGetKnownFolderPath(&FOLDERID_LocalAppDataLow, 0, NULL, &wszPath);
	if (hr == S_OK && wszPath)
	{
		int alen = WideCharToMultiByte(CP_ACP, 0, wszPath, -1, NULL, 0, NULL, NULL);
		if (alen > 0)
		{
			homedir = malloc(alen);
			if (homedir)
			{
				alen = WideCharToMultiByte(CP_ACP, 0, wszPath, -1, homedir, alen, NULL, NULL);
				if (alen <= 0)
				{
					free(homedir);
					homedir = NULL;
	...
	if (homedir == NULL)
		return SC_ERROR_INTERNAL;
	rv = snprintf(buf, bufsize, "%s/%s", homedir, cache_dir);
