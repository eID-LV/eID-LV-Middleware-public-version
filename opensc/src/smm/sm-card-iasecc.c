...
{
	struct sm_cwa_session *cwa_session;
	...
	if (sm_info->session.cwa.params.crt_at.algo.algo_id_len != 1)
		LOG_FUNC_RETURN(ctx, SC_ERROR_NOT_SUPPORTED);

	cwa_session = &sm_info->session.cwa;
	cwa_keyset = &sm_info->session.cwa.cwa_keyset;
	...
	rv = sm_cwa_init_session_keys(ctx, cwa_session, cwa_session->params.crt_at.algo.algo_id[0]);
