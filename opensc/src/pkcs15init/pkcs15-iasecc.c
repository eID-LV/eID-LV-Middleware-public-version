	...
	if (rv == SC_ERROR_DATA_OBJECT_NOT_FOUND)   {
		...
		*(sdo->docp.size.value + 1) = sz & 0xFF;
		...
		if (card->type == SC_CARD_TYPE_IASECC_OBERTHUR)   {
			...
			sdo->data.pub_key.rsa.cha.value = calloc(1, 2);
			if (!sdo->data.pub_key.rsa.cha.value)
				LOG_FUNC_RETURN(ctx, SC_ERROR_OUT_OF_MEMORY);
			sdo->data.pub_key.rsa.cha.size = 2;
			sdo->data.pub_key.rsa.cha.tag = IASECC_SDO_PUBKEY_TAG_CHA;
		}
		...
		struct sc_pkcs15_object *object, struct sc_pkcs15_pubkey *pubkey)
{
	...
	pubkey->u.rsa.modulus.len = sdo_pubkey->data.pub_key.rsa.n.size;
	pubkey->u.rsa.modulus.data  = (unsigned char *) malloc(pubkey->u.rsa.modulus.len);
	if (!pubkey->u.rsa.modulus.data)
		LOG_FUNC_RETURN(ctx, SC_ERROR_OUT_OF_MEMORY);
	memcpy(pubkey->u.rsa.modulus.data, sdo_pubkey->data.pub_key.rsa.n.value, pubkey->u.rsa.modulus.len);

	pubkey->u.rsa.exponent.len = sdo_pubkey->data.pub_key.rsa.e.size;
	pubkey->u.rsa.exponent.data = (unsigned char *) malloc(pubkey->u.rsa.exponent.len);
	if (!pubkey->u.rsa.exponent.data)
		LOG_FUNC_RETURN(ctx, SC_ERROR_OUT_OF_MEMORY);
	memcpy(pubkey->u.rsa.exponent.data, sdo_pubkey->data.pub_key.rsa.e.value, pubkey->u.rsa.exponent.len);
		...
		int sdo_class, int ref)
{
	...
	if (sdo->sdo_class == IASECC_SDO_CLASS_RSA_PUBLIC)   {
		if (sdo->data.pub_key.rsa.cha.value)   {
			...
			sdo->data.pub_key.rsa.cha.value = NULL;
			sdo->data.pub_key.rsa.cha.size = 0;
		}
	}
