...
#endif
		if (card->flags & SC_CARD_FLAG_GETRESP_NO_SM)
		{
			/* we make room for additional data returned by unencrypted GET RESPONSE */
			sm_apdu->resplen *= 2;
		}
	}
		...
		const sc_apdu_t *sm_apdu, sc_apdu_t *apdu)
{
	...
	/* in case SM is broken and no data retuned, copy status word to original APDU
	 * in order to information the application
	 */
