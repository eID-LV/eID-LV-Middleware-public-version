...
class OpenSCToken : public Tokend::ISO7816Token
{
	...
	public:
		sc_context_t *mScCtx;
		sc_card_t *mScCard;
		sc_pkcs15_card_t *mScP15Cards[MAX_P15_ADF];

	private:
		int opensc_bind(struct sc_app_info *app_info);
		int get_objects(unsigned int type, struct sc_pkcs15_object **enum_objs, int ret_count);
		void populate();
